...
public class EquipeServiceImpl implements EquipeService {
    ...
    public EquipeDTO addEquipe(EquipeDTO equipeDTO) {
        ...
        // Vérifier si equipeDTO est null
        if (equipeDTO == null) {
            throw new IllegalArgumentException("L'équipe ne peut pas être null.");
        }

        // Vérifier si le nom de l'équipe est null ou vide
        ...
        // Sauvegarder l'équipe avec la couleur
    ...
    public List<EquipeDTO> getAllEquipes() {
        ...
        // Si aucune équipe n'est trouvée, retourner une liste vide au lieu de lever une exception
    ...
    public EquipeDTO updateEquipe(UUID code, EquipeDTO equipeDTO) {
        ...
        Equipe equipe = equipeRepository.findById(code)
                .orElseThrow(() -> new RessourceNotFoundException("L'équipe avec le code spécifié n'existe pas."));

        // Mettre à jour les informations de l'équipe
        equipe.setNom(equipeDTO.getNom());
        equipe.setDescription(equipeDTO.getDescription());
        equipe.setCouleur(equipeDTO.getCouleur()); // Mise à jour de la couleur
